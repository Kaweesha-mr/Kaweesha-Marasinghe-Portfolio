...
export default function Home() {
  ...
  const [campusPaused, setCampusPaused] = useState(false);

  useEffect(() => {
    if (campusPaused) return;
    const timer = window.setInterval(() => {
      setActiveCampusIndex((index) => (index + 1) % campusMoments.length);
    }, 5200);
