...
const toolkit = [
  { number: "01", title: "Application engineering", label: "BUILD", detail: "Production-minded services, APIs, and product experiences.", technologies: ["Java", "Spring Boot", "Go", "Node.js"], icon: Server, tone: "blue" },
  { number: "02", title: "Interface systems", label: "SHAPE", detail: "Clear, responsive interfaces that make complex systems easier to use.", technologies: ["React", "Next.js", "TypeScript", "Tailwind"], icon: Code2, tone: "violet" },
  { number: "03", title: "Cloud & delivery", label: "SHIP", detail: "Infrastructure and release workflows designed for dependable delivery.", technologies: ["AWS", "Docker", "Kubernetes", "Terraform"], icon: CloudCog, tone: "cyan" },
  { number: "04", title: "Data & integration", label: "CONNECT", detail: "Reliable data flows and messaging between the parts of a platform.", technologies: ["PostgreSQL", "MongoDB", "Redis", "RabbitMQ"], icon: Database, tone: "orange" },
];
  ...
  return (
    <main>
      <header className={`site-header ${scrolled ? "is-scrolled" : ""}`}>
        ...
        <nav className={menuOpen ? "is-open" : ""} aria-label="Main navigation"><a href="#about" onClick={closeMenu}>About</a><a href="#experience" onClick={closeMenu}>Career</a><a href="#research" onClick={closeMenu}>Research</a><a href="#toolkit" onClick={closeMenu}>Tools</a><a href="#contact" onClick={closeMenu}>Contact</a></nav>
      ...
      <section className="toolkit toolkit-premium section-space" id="toolkit"><div className="page-width"><div className="section-kicker light"><span>05</span><span>Technical toolkit / the systems behind the work</span></div><div className="toolkit-head"><div><h2>Tools I use<br /><span>to make things work.</span></h2></div><p>A practical stack across application code, infrastructure, data, and delivery—chosen to turn complex requirements into dependable software.</p></div><div className="toolkit-grid">{toolkit.map((item) => <article className={`toolkit-item toolkit-${item.tone}`} key={item.title}><div className="toolkit-item-top"><span className="toolkit-number">{item.number}</span><IconBadge icon={item.icon} tone="dark" /><span className="toolkit-label">{item.label}</span></div><h3>{item.title}</h3><p>{item.detail}</p><div className="toolkit-techs">{item.technologies.map((technology) => <span key={technology}>{technology}</span>)}</div></article>)}</div><div className="toolkit-foot"><span><Workflow size={15} /> FROM IDEA TO RELEASE</span><b>CODE · CONNECT · OBSERVE · IMPROVE</b></div></div></section>

      <section className="community campus-life section-space" id="campus"><div className="page-width"><div className="section-head"><div><div className="section-kicker"><span>06</span><span>Campus life / an illustrated archive</span></div><h2>The work<br /><span>behind the work.</span></h2></div><p>A private collection of recognition, mentorship, and the people who shaped my SLIIT years.</p></div><div className="campus-intro"><div><span className="campus-signal"><i /> ARCHIVE / SLIIT / 2023—2026</span><p>Before the job titles and production systems, there were rooms full of people, questions, certificates, and small moments of showing up.</p></div><div className="campus-count"><strong>08</strong><span>selected<br />frames</span></div></div><div className="campus-feature"><figure className="campus-feature-image"><div className="campus-feature-image-stage" key={activeCampusMoment.src}><div className="campus-image-wrap"><Image src={activeCampusMoment.src} alt={activeCampusMoment.alt} width={1600} height={1200} priority={activeCampusIndex === 4} quality={95} /></div><figcaption><span>{String(activeCampusIndex + 1).padStart(2, "0")} / {activeCampusMoment.label}</span><strong>{activeCampusMoment.title}</strong><p>{activeCampusMoment.detail}</p></figcaption></div><div className="campus-feature-controls"><button type="button" onClick={showPreviousCampusMoment} aria-label="Show previous campus moment"><ChevronLeft size={17} /></button><span>{String(activeCampusIndex + 1).padStart(2, "0")} <i /> {String(campusMoments.length).padStart(2, "0")}</span><button type="button" onClick={showNextCampusMoment} aria-label="Show next campus moment"><ChevronRight size={17} /></button></div></figure><div className="campus-feature-copy"><span className="campus-signal"><i /> ARCHIVE / SELECT A FRAME</span><h3>Achievement is<br /><span>never a solo story.</span></h3><p>Every academic milestone carries the support of friends, mentors, family, and the communities that make effort feel worthwhile.</p><div className="campus-rule"><span>SLIIT / FACULTY OF COMPUTING</span><span>FRAME {String(activeCampusIndex + 1).padStart(2, "0")} / 08</span></div></div></div><div className="campus-rail">{campusMoments.map((moment, index) => <button type="button" className={`campus-rail-item ${index === activeCampusIndex ? "is-active" : ""}`} key={moment.src} onClick={() => showCampusMoment(index)} aria-label={`Show ${moment.title}`}><span className="campus-rail-image"><Image src={moment.src} alt={moment.alt} width={1600} height={1200} quality={82} /></span><span className="campus-rail-caption"><span>{String(index + 1).padStart(2, "0")} / {moment.label}</span><strong>{moment.title}</strong></span></button>)}</div></div></section>

      <section className="contact contact-luxury page-width" id="contact"><div className="contact-shell"><div className="contact-particles" aria-hidden="true"><i /><i /><i /><i /><i /><i /><i /><i /></div><div className="contact-primary"><div className="section-kicker light"><span>07</span><span>Contact / let&apos;s make something dependable</span></div><h2>Let&apos;s build<br /><span>what matters next.</span></h2><p>For thoughtful products, ambitious technical challenges, or a conversation about software engineering and research.</p><a className="contact-email" href="mailto:kaweesha.mr@gmail.com"><span className="contact-email-icon"><Mail size={18} /></span><span><small>EMAIL ME DIRECTLY</small><strong>kaweesha.mr@gmail.com</strong></span><ArrowUpRight size={19} /></a></div><aside className="contact-aside"><span className="contact-availability"><i /> OPEN TO A GOOD CONVERSATION</span><div className="contact-aside-rule" /><h3>From first idea<br /><span>to reliable release.</span></h3><p>Tell me what you&apos;re building, what&apos;s difficult, or where the next opportunity is. I&apos;ll bring curiosity, clarity, and a practical engineering mindset.</p><div className="contact-meta"><div><span>BASED IN</span><strong>Colombo, Sri Lanka</strong></div><div><span>RESPONSE</span><strong>Thoughtful and direct</strong></div></div></aside><div className="contact-bottom"><span>CONNECT ELSEWHERE</span><div className="contact-links"><a href="https://www.linkedin.com/in/kaweeshamr/" target="_blank" rel="noreferrer"><Network size={15} /> LinkedIn <ExternalLink size={12} /></a><a href="https://github.com/Kaweesha-mr" target="_blank" rel="noreferrer"><GitBranch size={15} /> GitHub <ExternalLink size={12} /></a><a href="tel:+94770723273"><Phone size={15} /> +94 77 072 3273</a></div><span className="contact-mark">KM / 2026</span></div></div></section>

      <footer className="footer page-width"><div className="footer-brand"><span className="footer-brand-mark" /><span>© 2026 KAWEESHA MARASINGHE</span></div><div className="footer-status"><i /><span>BUILDING WITH INTENT</span><b /></div><a href="#top">BACK TO TOP <ArrowUpRight size={12} /></a></footer>
