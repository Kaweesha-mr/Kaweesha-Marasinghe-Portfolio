...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section className="experience experience-dark section-space" id="experience"><div className="page-width"><div className="section-head"><div><div className="section-kicker"><span>02</span><span>Professional experience / career orbit</span></div><h2>A clear path<br /><span>through engineering.</span></h2></div><p>Four chapters of building, debugging, learning, and moving real systems forward.</p></div><div className="experience-timeline"><div className="timeline-spine" />{roles.map((role, index) => <article className={`experience-entry ${role.current ? "current" : ""}`} key={`${role.company}-${role.dates}`}><div className="experience-marker"><span>{String(index + 1).padStart(2, "0")}</span><i /></div><div className="experience-card"><div className="experience-card-top"><div className="company-lockup"><span className={`company-logo ${role.tone}`}>{role.mark}</span><div><strong>{role.brand}</strong><small>{role.company}</small></div></div><span className="experience-date">{role.dates}</span></div><h3>{role.role}</h3><p className="experience-summary">{role.description}</p><ul>{role.highlights.map((highlight) => <li key={highlight}><CheckCircle2 size={14} /> <span>{highlight}</span></li>)}</ul><div className="experience-tags">{role.stack.map((item) => <span key={item}>{item}</span>)}</div></div></article>)}</div></div></section>
